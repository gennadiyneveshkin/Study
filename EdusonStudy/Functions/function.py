...
# TODO Пример с локальными переменными
# здесь data - локальная переменная, заданная только для функции process
# data изменяется и принимает те значения, которые присваиваются ей с помощью записи process(<то, чем станет data>),
# в наших примерах это nums и nums2
...
result = process(nums)
print(f'''
Результат функции process, 
которая обрабатывает переменную data, 
переменноай data присваивается значение переменной nums с помощью записи process(nums)
и результат сохраняется в result
{result}''')

nums2 = [1, 1, 1, 1, 1]
result2 = process(nums2)
print(f'''
Результат функции process, 
которая обрабатывает переменную data, 
переменноай data присваивается значение переменной nums2 с помощью записи process(nums2)
и результат сохраняется в result
{result2}''')
